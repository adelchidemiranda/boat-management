...
const createStatements = [
    ...
    plan VARCHAR(20) DEFAULT 'base' CHECK (plan IN ('base', 'premium', 'enterprise')),
    ...
    name VARCHAR(100) NOT NULL,
    hin VARCHAR(50) UNIQUE NOT NULL,
    model VARCHAR(100) NOT NULL,
    manufacturer VARCHAR(100) NOT NULL,
    year INTEGER NOT NULL,
    length_meters DECIMAL(6,2),
    engine_type VARCHAR(100),
    engine_hours DECIMAL(8,1) DEFAULT 0,
...
const seedStatements = [
    `INSERT INTO manufacturers (name, country, contact_email, plan) VALUES
  ...
  ON CONFLICT DO NOTHING`,
    `INSERT INTO yachts (manufacturer_id, name, hin, model, manufacturer, year, length_meters, engine_type, engine_hours, owner_name, owner_email, health_score, status, location_lat, location_lng) VALUES
  ...
  ON CONFLICT DO NOTHING`,
    `INSERT INTO alerts (yacht_id, type, message, component, resolved) VALUES
  ...
  ON CONFLICT DO NOTHING`,
    `INSERT INTO maintenance_records (yacht_id, type, description, status, technician, cost, scheduled_date, completed_date) VALUES
  ...
  ON CONFLICT DO NOTHING`,
    `INSERT INTO documents (yacht_id, type, name, expiry_date, status) VALUES
  ...
  ON CONFLICT DO NOTHING`,
    `INSERT INTO tickets (yacht_id, title, description, priority, status, assigned_to, estimated_cost) VALUES
...
const allStatements = [...createStatements, ...seedStatements];
console.log(`Applying ${allStatements.length} statements to NeonDB...\n`);

let success = 0;
for (const stmt of allStatements) {
    try {
        await sql.unsafe(stmt);
        const preview = stmt.trim().substring(0, 55).replace(/\s+/g, ' ');
        console.log(`  ✓ ${preview}...`);
        success++;
    } catch (e) {
        const preview = stmt.trim().substring(0, 55).replace(/\s+/g, ' ');
        console.error(`  ✗ [${e.code}] ${preview}...`);
        console.error(`    ${e.message}`);
    }
}

console.log(`\n✅ Complete! ${success}/${allStatements.length} statements applied.`);
console.log('Your NeonDB is ready.');
